...
/**
 * Plugin 'guestbook' for the 'tx_guestbook' extension.
 *
 * @author	Udo von Eynern <udo@voneynern.de>
 * @package TYPO3
 * @subpackage tx_guestbook
 */
class tx_guestbook_pi1 extends tslib_pibase {
	var $prefixId = "tx_guestbook_pi1";		// Same as class name
	var $scriptRelPath = "pi1/class.tx_guestbook_pi1.php";	// Path to this script relative to the extension dir.
	var $extKey = "tx_guestbook";	// The extension key.
	var $strEntryTable = "tx_guestbook_entries";
	var $code = '';
	var $tt_news=array();
	var $for_tt_news;
	...
	function init($conf) {
		...
		$this->templateCode = $this->cObj->fileResource($templateflex_file  ? 'uploads/tx_guestbook/'.$templateflex_file : $this->conf['templateFile']);
	...
	function displayList() {
		...
		if ($count > 0) {
			...
			if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tx_guestbook']['extraItemMarkerHook'])) {
				foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tx_guestbook']['extraItemMarkerHook'] as $_classRef) {
	...
	function getPageBrowser($markerArray) {
		...
		if ($newsCount > $begin_at + $this->config['limit']) {
			...
			$next_link = $this->pi_linkTP_keepPIvars($this->pi_getLL('pi_list_browseresults_next', 'Next >'), $params, $this->allowCaching);
		...
		if ($begin_at) {
			...
			$prev_link = $this->pi_linkTP_keepPIvars($this->pi_getLL('pi_list_browseresults_prev', '< Previous'), $params, $this->allowCaching);
		...
		for ($i = $firstPage ; $i < $lastPage; $i++) {
			if (($begin_at >= $i * $this->config['limit']) && ($begin_at < $i * $this->config['limit'] + $this->config['limit'])) {
				...
				$link = $this->pi_linkTP_keepPIvars($this->local_cObj->stdWrap($item, $this->conf['pageBrowser.']['pagelink_stdWrap.']) , $params, $this->allowCaching) . ' ';
	...
	function getItemMarkerArray($row) {
		...
		$substituteArray = array('firstname', 'surname', 'email', 'homepage', 'place', 'entry', 'entrycomment', 'submit');
		...
		if (!empty($row['homepage']) && $row['homepage'] != 'http://') {
			$markerArray["###GUESTBOOK_HOMEPAGE###"] = htmlspecialchars($this->get_url('', $row['homepage']));
		}

		$markerArray['###GUESTBOOK_DATE###'] = $this->local_cObj->stdWrap($row['crdate'], $this->conf['datetime_stdWrap.']);
		$markerArray['###GUESTBOOK_ONLYTIME###'] = $this->local_cObj->stdWrap($row['crdate'], $this->conf['date_stdWrap.']);
		$markerArray['###GUESTBOOK_ONLYDATE###'] = $this->local_cObj->stdWrap($row['crdate'], $this->conf['time_stdWrap.']);
	...
	function displayForm() {
		...
		{
			$substituteArray = array('firstname', 'surname', 'email', 'homepage', 'place', 'entry', 'submit', 'captcha_response');
			...
			$markerArray['###ACTION_URL###'] = $url;

			$this->postvars = t3lib_div::GPvar('tx_guestbook_pi1');

			if ($this->postvars['submitted'] == 1) {
				...
				if (!empty($error)) {
					...
					$db_fields = array('firstname', 'surname', 'email', 'homepage', 'place', 'entry');
			...
			if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tx_guestbook']['preEntryInsertHook'])) {
				foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tx_guestbook']['preEntryInsertHook'] as $_classRef) {
					$_procObj = & t3lib_div::getUserObj($_classRef);
					...
					if ($insert) {
						...
						if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tx_guestbook']['postEntryInsertedHook'])) {
							foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['tx_guestbook']['postEntryInsertedHook'] as $_classRef) {
								$_procObj = & t3lib_div::getUserObj($_classRef);
								$_procObj->postEntryInsertedProcessor($this);
							}
						}

						header('Location: '.t3lib_div::getIndpEnv('TYPO3_REQUEST_DIR').$this->getUrl($this->config['redirect_page']));
					}
				}
			}
	...
	function getEmailFromMail() {
		if (!empty($this->config['email_from_mail']))
			...
			return "tx_guestbook@".$_SERVER['SERVER_NAME'];;
	}
	...
	function sendFeedbackMail($emailto) {
		...
		$markerArray['###SERVER_NAME###'] = $_SERVER['SERVER_NAME'];
		$markerArray['###URL###'] =
			t3lib_div::getIndpEnv('TYPO3_SITE_URL').$this->getUrl($this->config['guestbook']);
...
if (defined("TYPO3_MODE") && $TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/tx_guestbook/pi1/class.tx_guestbook_pi1.php"])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]["XCLASS"]["ext/tx_guestbook/pi1/class.tx_guestbook_pi1.php"]);
}
